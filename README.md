...
NUME has one presentation layer and immutable published catalog/layout snapshots, with explicit runtime adapters:

| Target | Command | Storefront | Catalog validation/planning/update | Authentication / Stripe |
|---|---|---|---|---|
| Local development | `npm run dev` | Yes, with Vite HMR | Same-origin Worker API; updates require configured storage | Mock mode by default; live Stripe is never contacted |
| GitHub Pages | `npm run build:pages` | Yes, emitted to `out/` | Disabled by default; controls explain the limitation | None |
| Cloudflare/Sites | `npm run build:cloudflare` | Yes | Worker endpoint at `/api/merchant/catalog`; writes require `CATALOG_DB` | Secret bindings and strict origin allowlist |

`npm run build` means the primary, full Cloudflare/Sites build. It generates `dist/server/index.js` and `dist/.openai/hosting.json`. `npm run deploy:cloudflare` is the explicit manual deployment step; CI does not deploy Cloudflare.

### Runtime and API boundaries

Shared browser code imports only serializable catalog types, the published snapshots, marketplace components, navigation, styling, and the deployment-aware asset resolver. Server code lives under `server/`; `worker/index.ts` exposes `/api/health` and `/api/merchant/catalog`. There is intentionally no `app/api` directory, so Next static export cannot discover a Node route. Local and Cloudflare use the same same-origin URL.

GitHub Pages cannot execute authentication, Stripe, webhooks, databases, audit writes, or background jobs. Its default capability profile therefore disables administration and reads `data/catalog-sync/published-marketplace.v1.json` plus `published-layout.v1.json` at build time. An optional remote API can be enabled with the public build variable `NEXT_PUBLIC_NUME_API_BASE_URL` (an absolute HTTPS origin); authenticated cross-origin operations additionally require the server secret `NUME_CORS_ORIGINS` to contain that exact origin. Wildcard credentialed CORS is not supported.

Server bindings/secret names (never values) are `NUME_MERCHANT_TOKEN`, `NUME_CORS_ORIGINS`, and `CATALOG_DB`. Future Stripe integration must use a server-only secret such as `STRIPE_SECRET_KEY`; no Stripe secret is read by the frontend. Cloudflare storage has no filesystem assumption. Local mode identifies itself as `local-mock`, validates and plans safely, and refuses persistence until a storage adapter is configured.

### Catalog publication

Editable files under `data/vendor-catalogs/` produce validated plans. Publication creates immutable files under `data/catalog-sync/`; runtime storefront reads use only those published files. GitHub Pages cannot persist an edit: commit a newly validated snapshot and push it to `main` to publish it through Pages. The source layout is `data/layout/marketplace-layout.v1.json`; `npm run validate:catalog` fails unless its published copy is byte-for-byte equivalent as parsed JSON. Both store the canonical semantic label `Q&A`; CSS supplies vertical flow without reversing the string, preserving the accessible name.

The Pages base path is derived from `GITHUB_REPOSITORY`, or overridden with `NUME_PAGES_BASE_PATH`; no owner is hard-coded. `NEXT_PUBLIC_BASE_PATH` is generated by the build and the shared asset resolver applies it to every local image while leaving public HTTPS media unchanged.

### Verification and troubleshooting

Run `npm ci`, then `npm run validate:catalog`, `npm run validate:targets`, `npm run lint`, `npm run typecheck`, `npm test`, `npm run build:pages`, and `npm run build:cloudflare`. Re-run `npm run validate:targets` after either build to inspect artifacts for missing manifests, API output, Node-only references, secrets, and incorrect asset roots. If Pages fails, confirm `GITHUB_REPOSITORY` names this repository and that `out/` exists. If Cloudflare fails, confirm the Worker and hosting manifest exist and configure the D1-compatible `CATALOG_DB` plus secrets before enabling updates.
