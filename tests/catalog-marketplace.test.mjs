...
test("renderer retains selection, navigation, fallback chain, style isolation, and reduced motion", async () => { const [page,css]=await Promise.all([readFile(new URL("../app/page.tsx",import.meta.url),"utf8"),readFile(new URL("../app/globals.css",import.meta.url),"utf8")]); assert.match(page,/openWork\(work, rowIndex\)/); assert.match(page,/resolveRotundaMove/); assert.match(page,/data-nume-row/); const resilientImage=await readFile(new URL("../app/product-image.tsx",import.meta.url),"utf8"); assert.match(page,/ProductImage/); assert.match(resilientImage,/MAX_CONCURRENT = 3/); assert.match(resilientImage,/const DELAYS = \[750, 2000, 5000\]/); assert.match(css,/@media \(prefers-reduced-motion: reduce\)/); assert.match(css,/availability-sold_out/); });
