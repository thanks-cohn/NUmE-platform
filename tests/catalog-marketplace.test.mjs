...
test("renderer retains selection, navigation, fallback chain, style isolation, and reduced motion", async () => { const [page,responsive,tile,resilientImage]=await Promise.all([readFile(new URL("../app/page.tsx",import.meta.url),"utf8"),readFile(new URL("../app/styles/responsive.css",import.meta.url),"utf8"),readFile(new URL("../app/styles/product-tile.css",import.meta.url),"utf8"),readFile(new URL("../app/product-image.tsx",import.meta.url),"utf8")]); assert.match(page,/openWork\(work, rowIndex\)/); assert.match(page,/resolveRotundaMove/); assert.match(page,/data-nume-row/); assert.match(page,/ProductImage/); assert.match(resilientImage,/MAX_CONCURRENT = 3/); assert.match(resilientImage,/const DELAYS = \[750, 2000, 5000\]/); assert.match(responsive,/@media\(prefers-reduced-motion:reduce\)/); assert.match(tile,/availability-sold_out/); });

test("harmony tokens are validated and merchant styles remain selector-scoped", async () => {
  for (const profile of [...groups.groups, ...styles.profiles]) {
    for (const token of ["color_soft_surface", "color_edge", "title_composition", "image_treatment"]) assert.ok(profile.tokens[token], `${profile.style_profile_id} ${token}`);
  }
  const stylesheet = await readFile(new URL("../app/globals.css", import.meta.url), "utf8");
  assert.doesNotMatch(stylesheet, /(^|\})\s*\.tile\s*\{[^}]*merchant_/m);
});
